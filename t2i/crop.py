...
class MakeCutouts_v4(th.nn.Module):
    def __init__(self, cut_size, cutn, aug=True):
        super().__init__()
    ...
    def forward(self, input, augment=True, *args, **kwargs):
        ...
        for ch in range(self.cutn):
            if ch > self.cutn - self.cutn//4:
                ...
                size = int(max_size * th.zeros(1,).normal_(mean=.8, std=.3).clip(float(self.cut_size[0] / max_size), 1.))
                offsetx = th.randint(0, abs(sideX - size + 1), ())
                offsety = th.randint(0, abs(sideY - size + 1), ())
                cutout = input[:, :, offsety:offsety + size, offsetx:offsetx + size]

            if augment: cutout = self.augs(cutout)
