...
@click.command()
@click.pass_context
def cli(ctx, seed, seeds, experiment, prompt, device, video, verbose, **kwargs):
    ...
    for seed in seeds:
        ...
        for prompt in prompts:
            ...
            if kwargs['init'] == 'bigsleep':
                ...
                init = output_files[-1]
            elif kwargs['init'] == 'rudalle':
                from t2i.rudalle.__main__ import cli
                output_files = ctx.invoke(cli, seed=seed, prompt=prompt, batch_size=1)
                init = output_files[-1]
            elif kwargs['init'] == 'perlin':
                init, kwargs = 'perlin', dict(octaves=2**2, weight=22e-2, **kwargs)
            elif kwargs['init'] == 'pyramid':
                init, kwargs = 'pyramid',dict(octaves=2**3,  decay=99e-2, **kwargs)
            else:
                init = None
            #-----------------------------------------------------------------------
            ...
            for i,args in enumerate(refinement(**kwargs)):
                ...
                #-------------------------------------------------------------------
                vqlipse.optimizer(vqlipse.init_z(init=init, **args), **args)
                #-------------------------------------------------------------------
            ...
            th.cuda.empty_cache()
            #-----------------------------------------------------------------------
            if video: animate.video(outputs, config['video'], **kwargs)
        return outputs
